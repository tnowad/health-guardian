...
export const listPrescriptionsErrorResponseSchema = z.discriminatedUnion("type", [
...
export async function listPrescriptionsApi(query: ListPrescriptionsQuerySchema) {
    const response = await apiClient.get("/prescriptions", { params: query });
...
export function createListPrescriptionsQueryOptions(
    ...
    return queryOptions<
        ListPrescriptionsResponseSchema,
        ListPrescriptionsErrorResponseSchema,
        ListPrescriptionsQuerySchema,
        typeof queryKey
    >({
        queryKey,
        queryFn: () => listPrescriptionsApi(listPrescriptionsQuerySchema.parse(query)),
