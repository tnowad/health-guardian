...
;    }
    ...
    List<String> standardDosageEnum = Arrays.asList("500mg per dose, taken twice a day.", "500mg per dose, taken third a day.", "1 tablet before meals.");
    for (int i = 0; i < 300; i++) {
    ...
    for (int i = 0; i < 500; i++) {
      SideEffect sideEffect = SideEffect.builder()
        ...
        .severity(SideEffectSeverity.values()[random.nextInt(SideEffectSeverity.values().length)])
