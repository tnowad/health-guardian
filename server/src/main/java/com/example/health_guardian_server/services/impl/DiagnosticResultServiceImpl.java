...
public class DiagnosticResultServiceImpl implements DiagnosticResultService {
  ...
  @Override
  public Page<DiagnosticResultResponse> getAllDiagnosticResultsListDiagnosticResultsRequest(ListDiagnosticResultRequest request) {
    log.debug("Fetching all diagnostic results with request: {}", request);
    var diagnosticResults = diagnosticResultRepository.findAll(request.toSpecification(), request.toPageable())
  ...
  @Override
  public DiagnosticResultResponse updateDiagnosticResult(String id, CreateDiagnosticResultRequest request) {
    log.debug("Updating diagnostic result with id: {} and request: {}", id, request);
    var diagnosticResult = diagnosticResultRepository.findById(id)
        .
      orElseThrow(() -> {
        log.error("Diagnostic result not found with id: {}", id);
        return new ResourceNotFoundException("Diagnostic result not found with id: " + id);
